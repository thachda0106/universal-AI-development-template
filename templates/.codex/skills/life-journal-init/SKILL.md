...
> **Duration**: 5 minutes
> **Config**: Read `journal_path` from `.codex/docs/life-config.yaml`
...
1. **Check if journal already exists**: Does the `{journal_path}` exist with the full structure? (Read path from `.codex/docs/life-config.yaml`)
...
2. **Create config** `journal.yaml` (read `journal_path` from `.codex/docs/life-config.yaml`):
   ```yaml
